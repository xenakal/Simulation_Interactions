    ...
    def takePicture(self,targetList):
        ...
        for target in targetList:
             ...
             if(math.cos(self.alpha+self.beta/2) > 0 and math.cos(self.alpha-self.beta/2) > 0):
                ...
                if((self.xc-target.xc < 0 and margin_left <= 0 ) or ((d1 <= target.size or d2 <= target.size) and self.yc > target.yc)):
                    #print("5")
                    targetInTriangle.append(target)
                ...
                if((self.xc-target.xc < 0 and margin_right <= 0) or ((d1 <= target.size or d2 <= target.size) and self.yc < target.yc)):
                    #print("7")
                    targetInTriangle.append(target)
        ...
        for obj in orderedTarget:
            ...
            for targetAlreadyDetected in self.targetDetectedList:
                ...
                cdt1 = (d2 < d0 and d2 > d1) 
                cdt2 = (d2 > d0 and d2 < d1)
                cdt3 = (d3 < d0 and d3 > d1)
                cdt4 = (d3 > d0 and d3 < d1)
                ...
                #the object is partially hiden
                    ...
                    projection = numpy.array([proj_p1[0],proj_p1[1],projection[2],projection[3],target.id])          
                    hidden = 1
                    break
                else:
            ...
            if (hidden == 0 or hidden == 1):
                self.targetDetectedList.append(numpy.array([target,actuall_projection,hidden]))
