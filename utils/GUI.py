...
class GUI:
    ...
    def drawTarget(self, x, y, size, tar_id, label, tab):
        ...
        if tab[0] <= x + size <= tab[0] + tab[2] and tab[1] <= y + size <= tab[1] + tab[3]:
            # render text
    ...
    def drawCam(self, x, y, alpha, beta, cam_id, color=0, l=100):
        ...
        pygame.draw.line(self.screen, CAMERA, (x, y),
                         (x + l * math.cos(alpha - beta / 2), y + l * math.sin(alpha - beta / 2)), 2)
        pygame.draw.line(self.screen, CAMERA, (x, y),
                         (x + l * math.cos(alpha + beta / 2), y + l * math.sin(alpha + beta / 2)), 2)
        ...
        for camera in myRoom.cameras:
            ...
            for projection in camera.projections:
                ...
                if m > 0:
                    ...
                    pygame.draw.circle(self.screen, (100 + m * 30, 255 - m * 30, 255),
                                       (x_off + 85 + d0, y_off + 8 + n * 30), 5)
                    pygame.draw.circle(self.screen, (100 + m * 30, 255 - m * 30, 255),
                                       (x_off + 85 + d1, y_off + 8 + n * 30), 5)
                    ...
                    if m > 1:
                        pygame.draw.line(self.screen, (100 + m * 30, 255 - m * 30, 255),
                                         (x_off + 85 + d0, y_off + 8 + n * 30), (x_off + 85 + d1, y_off + 8 + n * 30),
                                         2)
