...
class GUI:
    ...
    def drawTarget(self,targets,tab):
        for target in targets:
            ...
            if(target.xc+target.size >= tab[0] and target.xc+target.size <= tab[0]+tab[2] and target.yc+target.size >= tab[1] and target.yc+target.size <= tab[1]+tab[3]):
                # render tetarget.xct
                ...
                if(target.size >= 5):
                    pygame.draw.circle(self.screen,target.color,(target.xc,target.yc),math.floor(target.size-0.5*target.size))
    ...
    def drawCam(self,cameras,color = 0 , l = 100):
        for camera in cameras: 
            ...
            pygame.draw.line(self.screen,CAMERA,(camera.xc,camera.yc),(camera.xc+l*math.cos(camera.alpha-camera.beta/2),
    ...
    def drawProjection(self,myRoom):
        ...
        for camera in myRoom.cameras:
            ...
            for target in detected:
                ...
                pygame.draw.line(self.screen,target[0].color,(x_off + 85 + d0,y_off+8+n*30),(x_off+85+d1,y_off+8+n*30), 2)
