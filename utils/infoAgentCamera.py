...
class TargetEstimatorList:
    def __init__(self, nTime=20, current_time=0):
        ...
        self.estimatorList = []  # [[TargetInfo],[TargetInfo]...]
    ...
    def add_create_target_estimator(self,room,time_from_estimation,target_ID,agent_ID,seenByCam):
        for target in room.targets:
            if str(target.id) == str(target_ID):
                ...
                if not self.is_target_estimator(estimator):
                     self.estimatorList.append(estimator)
    ...
    def is_target_estimator(self,target_estimator):
        for estimator in self.estimatorList:
    ...
    def get_estimator_from_agent(self,agent_ID):
        estimator_list_agent = []
        for estimator in self.estimatorList:
            if estimator.agent_ID == agent_ID:
                estimator_list_agent.append(estimator)
        return  estimator_list_agent

    def get_estimator_target(self,target_ID):
        estimator_list_target = []
        for estimator in self.estimatorList:
            if estimator.agent_ID == target_ID:
                estimator_list_target.append(estimator)
        return  estimator_list_target
    ...
    def get_estimator_time_target_agent(self,time,target_ID,agent_ID):
        ...
        for estimator in self.estimatorList:
            if time != -1:
                cdt1 = estimator.timeStamp == time
            if target_ID != -1:
                cdt2 = estimator.agent_ID == agent_ID
            if agent_ID != -1:
                cdt3 = estimator.agent_ID == target_ID
